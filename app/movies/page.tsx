...
export default async function MoviesPage({ searchParams }: PageProps) {
    ...
    const conditions: string[] = [`type = 'MOVIE'`, visClause];
    const values: any[] = [];
    let idx = 1;
    ...
    let movies: any[] = await prisma.$queryRawUnsafe(
    ...
    if (sort === "rating") movies = movies.sort((a, b) => parseFloat(b.rating || "0") - parseFloat(a.rating || "0"));

    const meta = await prisma.$queryRawUnsafe<any[]>(
        `SELECT genre, year FROM movie WHERE type = 'MOVIE' AND ${visClause}`
    );
    ...
    return (
        <div style={{ padding: "2rem 4% 4rem" }}>
            <h1 style={{ color: "white", fontSize: "2rem", fontWeight: "800", marginBottom: "1.5rem" }}>
                Películas
                {movies.length > 0 && <span style={{ fontSize: "1rem", fontWeight: "400", color: "rgba(255,255,255,0.35)", marginLeft: "12px" }}>{movies.length} resultado{movies.length !== 1 ? "s" : ""}</span>}
            </h1>
