...
export default async function AnimesPage({ searchParams }: PageProps) {
    ...
    const conditions: string[] = [`type = 'ANIME'`, `(hidden IS NULL OR hidden = false)`];
    const values: any[] = [];
    let idx = 1;
    ...
    let animes: any[] = await prisma.$queryRawUnsafe(sql, ...values);
    ...
    const meta = await prisma.$queryRawUnsafe<any[]>(
        `SELECT genre, year FROM movie WHERE type = 'ANIME' AND (hidden IS NULL OR hidden = false)`
    );
