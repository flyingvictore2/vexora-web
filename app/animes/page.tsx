...
export default async function AnimesPage({ searchParams }: PageProps) {
    ...
    const conditions: string[] = [`type = 'ANIME'`, visClause];
    const values: any[] = [];
    let idx = 1;
    ...
    let animes: any[] = await prisma.$queryRawUnsafe(
        `SELECT * FROM movie WHERE ${conditions.join(" AND ")} ORDER BY ${orderSQL}`, ...values
    );
    animes = animes.map(m => ({ ...m, year: Number(m.year), views: Number(m.views ?? 0) }));
    if (sort === "rating") animes = animes.sort((a, b) => parseFloat(b.rating || "0") - parseFloat(a.rating || "0"));

    const meta = await prisma.$queryRawUnsafe<any[]>(
        `SELECT genre, year FROM movie WHERE type = 'ANIME' AND ${visClause}`
    );
    ...
    return (
        <div style={{ padding: "2rem 4% 4rem" }}>
            <h1 style={{ color: "white", fontSize: "2rem", fontWeight: "800", marginBottom: "1.5rem" }}>
                Anime
                {animes.length > 0 && <span style={{ fontSize: "1rem", fontWeight: "400", color: "rgba(255,255,255,0.35)", marginLeft: "12px" }}>{animes.length} resultado{animes.length !== 1 ? "s" : ""}</span>}
            </h1>
